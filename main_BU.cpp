...
int main(int argc, char** argv) {
	...
	int HALMode = 1; // original BU algorithm
	...
	if (status==2) {
		// show the help page
		options.outputHALBUUsage(argv[0], 0);
		exit(1);
	}
	...
	int num_chars_to_consider_in_model = 4; // A,C,G,T
	performRAL(&options, rateGrpFile, preChkptFile,num_chars_to_consider_in_model);
